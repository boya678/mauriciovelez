...
EXCEL_FILE = os.path.join(os.path.dirname(__file__), "Conferencia 23082026 Julian.xlsx")
...
# col1=número destino  col2=estado  col3=message_id
pendientes: list[tuple] = []
for row in ws.iter_rows(min_row=2):
    cell_numero = row[0]
    cell_estado = row[1] if len(row) > 1 else ws.cell(row=cell_numero.row, column=2)
    cell_msgid = row[2] if len(row) > 2 else ws.cell(row=cell_numero.row, column=3)
    ...
    pendientes.append((numero_str, cell_estado, cell_msgid))
...
async def enviar(session: aiohttp.ClientSession, semaphore: asyncio.Semaphore,
                 numero_str: str, cell_estado, cell_msgid) -> None:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        tareas = [
            enviar(session, semaphore, num, cel_estado, cel_msgid)
            for num, cel_estado, cel_msgid in pendientes
        ]
